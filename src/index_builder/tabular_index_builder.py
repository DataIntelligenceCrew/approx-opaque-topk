...
def kmeans_clustering(df: pd.DataFrame, n_clusters: int, subsample_size: int, exclude_columns: List[str],
                      seed: int = 42) -> Tuple[List[pd.DataFrame], np.ndarray]:
    """
    ...
    subsample_df: pd.DataFrame = df.sample(n=subsample_size, random_state=seed)
...
def main(file_path: str, n_clusters: int, tree_output_path: str, single_node_tree_output_path: str,
         subsample_size: int, id_column: str, pred_column: str, seed: int = 42):
    """
    ...
    :param pred_column: The name of the column in which to predict the ID.
    ...
    clusters, centroids = kmeans_clustering(df, n_clusters, subsample_size, exclude_columns, seed)
